...
`statsReg` bit0 (Interrupt OK) が1のときのみディスパッチされ、最下位ビットの割り込みが
優先される (`bits.TrailingZeros64`)。ディスパッチ時は `IP` → `CS` → `Flags` の順で
スタックにpushしてからハンドラへジャンプし、`statsReg` bit0 をクリアする。
...
| `0x21` | **RET** | `RET [near, far]` | Pops `CS` then `IP`. The operand is accepted but ignored (kept for backward compatibility) — always matches whatever `CALL` form was used. |
| `0x22` | **IRET** | `IRET [NULL] [NULL] [NULL]` | Pops `Flags`, then `CS`, then `IP`. |
