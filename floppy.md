...
ff58   0e 0f d8 00   JMP -36                ff34 に戻って続きを読む
...
⚠️ 注意点（自分で直すとき用のメモ）:
- `ff5c` の `OUT 0x10, cx` はコメント上「RESET Floppy」だが、この時点で `cx == 2`
  なので実際に書かれるのは `0x02`（WRITE）。RESET は `0x03`。**要確認/たぶんバグ**。
...
## 6. occ 側で必要になりそうな拡張（未実装・メモ）

- `.org` / 転送先アドレスをフラグで可変化（現状 `codeOrg = 0x7c00` ハードコード）。
  stage-2 を別アドレスに置きたくなったら。`oasm link` の `@offset` と整合させる。
