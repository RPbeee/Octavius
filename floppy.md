...
- 1セクタ=512バイト。`headbyte` は 0..511 を転送でき、512 到達後のアクセスで
  `STAT |= error`。1セクタ読み書きしたら `RESET`（CMD=0x03）で
  `args`/`headbyte`/`STAT` をクリアして次へ。
  （※以前は `updateFloppyIO` の境界が `headbyte < 511` で **WRITE の 512 バイト目
  (index 511) が書けない** off-by-one があった。READ は `floppyTick` が別途
  `DATA` を載せるため 512 バイト読めていたので露見していなかった。`< 512` に修正し、
  READ 側の `DATA` 参照に `headbyte < 512` の OOB ガードを追加済み。）
...
4. **ロード先が 0x7c00 を跨ぐ場合の罠**: stage-1 自身は ROM により 0x7c00（ページ 0x7c）
   に置かれて実行される。カーネルを 0x7c00 より下（例 0x5000）から連続ロードすると
   ロード範囲が **自分自身を上書き**してクラッシュする。対策は stage-1 を安全な低位ページ
   （カーネルがロードしない領域, 例 0x0e00）へ**自己コピーしてそこで実行**すること。
   oasm の near ジャンプ(mode 0x1f, cs 相対・下位バイトのみ)と `JMP rel`/`JZ`/`JNZ` は
   位置独立なので、1 ページに収まる stage-1 はそのままコピーして走らせられる（絶対
   ジャンプはカーネルへの `JMP far` だけにする）。実例は OctOS の `playGround/src/boot.s`。
...
- ~~インラインアセンブラ（`asm("...")`）があれば §3 の ARG 3連発を occ 内で表現できる~~
  → **`__diskcmd(cmd, c, h, s)` ビルトインとして実装済み**。RESET → ARG=0 →
  `OUT CMD` → `OUT ARG`×3 を「間に他命令を挟まず」emit する（各値は MSB を立てて
  渡すので 0 でも ARG≠0 と見なされる）。これで C/H/S 投入だけをハード直結にし、
  STAT ポーリングと 512B ストリーミングは素の occ (`__in`/`__out`) で書ける。
  OctOS の `blk_read`/`blk_write`（`playGround/src/kernel.c`）が実例。
